...
def apply_scail_continuity_core(
    *,
    ...
    # Same luma match as prepend_continuity_source — keep lock + conditioning aligned.
    if source_luma_ref is not None and int(source_luma_ref.numel()) > 0:
        ref = source_luma_ref[0] if source_luma_ref.dim() == 4 else source_luma_ref
        tail_clip = normalize_guide_luma_to_source(
            tail_clip,
            ref,
            width=width,
            height=height,
    ...
    if scail_ok:
        settle = resolve_continuity_settling_frames()
        note = (
            f"  continuity branch seg #{seg.index + 1}: source-prepend + SCAIL lock "
            f"{lock_px}f ({t_lock} latent, kept through dual-stage) + settling {settle}f "
            f"(trimmed) + {ref_frames}f ref (overlap {plan.continuity_overlap_frames})"
        )
    else:
        note = (
